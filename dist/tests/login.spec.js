...
users.map((user) => {
    (0, test_1.test)(`Testing for ${user.role}`, async ({ page }) => {
        ...
        await (0, test_1.expect)(login).toBeVisible();
        await (0, test_1.expect)(login).toBeEnabled();
        ...
        user.role === "Local Admin"
            ? await (0, test_1.expect)(reports).not.toBeVisible()
            : await (0, test_1.expect)(reports).toBeVisible();
        const officers = page.getByRole("link", { name: "Officers" });
        user.role == "Local Admin"
            ? await (0, test_1.expect)(officers).not.toBeVisible()
            : await (0, test_1.expect)(officers).toBeVisible();
        ...
        await (0, test_1.expect)(page).toHaveURL(/signin/);
    });
});
